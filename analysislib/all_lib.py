...
if __name__ == '__main__':
    # df = pd.read_csv('../preprocess/all_lib.csv')
    # docs = []
    ...
    # np.save('alllib_docs.npy', np.array(docs))
    docs = np.load('alllib_docs.npy', allow_pickle=True).tolist()

    # Create a dictionary representation of the documents.
    # dictionary = Dictionary(docs)
    # dictionary.save('alllib.dict')
    # dictionary.filter_extremes(no_below=10, no_above=0.8)
    dictionary = Dictionary.load('alllib.dict')

    # Bag-of-words representation of the documents.
    # corpus = [dictionary.doc2bow(doc) for doc in docs]
    # np.save('alllib.npy', np.array(corpus))
    corpus = np.load('alllib.npy', allow_pickle=True).tolist()
