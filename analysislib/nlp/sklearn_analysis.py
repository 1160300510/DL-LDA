...
if __name__ == '__main__':
    ...
    np.save('sklearn_docs.npy', np.array(docs))
    # docs = np.load('sklearn_docs.npy', allow_pickle=True).tolist()

    # Create a dictionary representation of the documents.
    dictionary = Dictionary(docs)
    dictionary.filter_extremes(no_below=10, no_above=0.8)
    dictionary.save('sklearn.dict')
    # dictionary = Dictionary.load('sklearn.dict')

    # Bag-of-words representation of the documents.
    corpus = [dictionary.doc2bow(doc) for doc in docs]
    np.save('sklearn.npy', np.array(corpus))
    ...
    coherence_model_lda = CoherenceModel(model=model, texts=docs, corpus=corpus, dictionary=dictionary, coherence='c_v')
    coherence_lda = coherence_model_lda.get_coherence()
    #model.save('../model/nlp_sklearn/nlp_sklearn.model')
    model.save('nlp_sklearn_7.model')
